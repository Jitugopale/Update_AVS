    ...
    const fetchVerifiedUsers = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/branchUser/verified"
        );
      ...
      >
        <table style={{width:"100%",maxWidth:'100%'}}>
          ...
          {index + 1}
                  ...
                  <td style={{ padding: "8px", border: "1px solid #ddd" }}>{user.userName || "Name not available"}</td>
                  <td style={{ padding: "8px", border: "1px solid #ddd" }}>{user.branchId || "DOB not available"}</td>
                  <td style={{ padding: "8px", border: "1px solid #ddd" }}>{user.branchName || "DOB not available"}</td>
                  <td style={{ padding: "8px", border: "1px solid #ddd" }}>{user.mobileNumber || "DOB not available"}</td>
